...
- **Password-protected PDFs aren't supported.** pdf-lib can't decrypt them, so metadata can't be read or edited while a PDF is locked — unlock it first with this site's [PDF Password Remover](../pdf-password-remover/index.html), then drop the result back in here.
